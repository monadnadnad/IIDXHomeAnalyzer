...
class LogAnalyzer:
    ...
    def __init__(self, log: Log):
        ...
        self.log = log
    def _compressed_positions(self, p: Player):
        # plotするとき扱いやすいよう[0, -MAX_PLAYER_NUM]に圧縮する
        positions = self.log.get_player_positions(p)
        for i in range(len(positions)):
            ...
            if pos == None:
                ...
                positions[i] = -pos
        return positions
    def get_status_changed(self, p: Player) -> list[bool]:
        """
        プレイした事が確定したタイミングを記録したリストを返す
        """
        times = self.log.get_times()
        positions = self.log.get_player_positions(p)
        N = len(positions)
        ret = [False]*N
        all_log = [self.log.get_log_at_time(times[i]) for i in range(N)]
        for i in range(N-1):
            p1 = positions[i]
            p2 = positions[i+1]
            log1 = all_log[i]
            ...
            #        break
        return ret
    def get_player_in_venue(self, p: Player) -> list[bool]:
        """
        プレイヤーがその時間にいた可能性をboolで表現したリスト
        """
        # 入店の確定位置を探す、最後の確定位置以降は退店しているとみなせる
        status_changed = self.get_status_changed(p)
        ...
        for i in range(last + 1, N):
            ret[i] = False
        return ret
    def get_players_over_time(self) -> list[set[Player]]:
        """
        各時間にいた可能性のあるプレイヤーの集合のリスト
        """
        N = len(self.log.get_times())
        ret = [set() for _ in range(N)]
        ps = self.log.get_players()
        player_in_venue = {}
        for p in ps:
            player_in_venue[p] = self.get_player_in_venue(p)
        for i in range(N):
            for p in ps:
                if player_in_venue[p][i]:
                    ret[i].add(p)
        return ret
    def plot_player_in_venue(self, p: Player, show=True):
        data = self.get_player_in_venue(p)
        #data = list(map(lambda x: -self.MAX_PLAYER_NUM*(not x), data))
        plt.plot(data, label="in venue")
        plt.legend()
        if show:
            plt.show()
    def plot_status_changed(self, p: Player, show=True):
        data = self.get_status_changed(p)
        plt.plot(data, label="status changed")
        plt.legend()
        if show:
            plt.show()
    def plot_positions(self, p: Player, show=True):
        positions = self._compressed_positions(p)
        plt.plot(positions, label="position")
        plt.legend()
        if show:
            plt.show()
    def plot_headcounts(self):
        times = self.log.get_times()
        playersets = self.get_players_over_time()
